...
PolynomialsBernardiRaugel<dim>::PolynomialsBernardiRaugel( const unsigned int k )
 ...
 polynomial_space_Q(create_polynomials_Q(k)),
 polynomial_space_bubble(create_polynomials_bubble(k))
{}

 
template <int dim>
std::vector< std::vector< Polynomials::Polynomial< double > > >
PolynomialsBernardiRaugel<dim>::create_polynomials_bubble (const unsigned int k)
{
...
template <int dim>
std::vector< std::vector< Polynomials::Polynomial< double > > >
PolynomialsBernardiRaugel<dim>::create_polynomials_Q (const unsigned int k)
{
  std::vector<std::vector< Polynomials::Polynomial< double > > > pols;
...
template <int dim>
void PolynomialsBernardiRaugel<dim>::compute(
  ...
  std::vector<Tensor<5, dim> > &fourth_derivatives) const
{
  ...
  // this seems like a waste of memory since information
  // is moved from these into the output vectors
  ...
  bubble_third_derivatives.resize((third_derivatives.size() == 0) ? 0 : n_bubbles);
  bubble_fourth_derivatives.resize((fourth_derivatives.size() == 0) ? 0 : n_bubbles);
  ...
  for (unsigned int i = 0; i < dim*GeometryInfo<dim>::vertices_per_cell; ++i)
    {
      ...
      if (third_derivatives.size() != 0)
        {
          third_derivatives[i] = outer_product(units[i%dim],Q_third_derivatives[i/dim]);
        }
     if (fourth_derivatives.size() != 0)
        {
          fourth_derivatives[i] = outer_product(units[i%dim],Q_fourth_derivatives[i/dim]);
        }
    }
  ...
  for (unsigned int i = dim*GeometryInfo<dim>::vertices_per_cell; 
                    ...
                    ++i)
  {
    unsigned int j = i - dim*GeometryInfo<dim>::vertices_per_cell; // ranges 0 to faces_per_cell-1
    if (values.size() != 0)
        {
      ...
      if (grad_grads.size() != 0)
        {
          grad_grads[i] = outer_product(normals[j],bubble_grad_grads[aniso_indices[j]]);
        }
      if (third_derivatives.size() != 0)
        {
          third_derivatives[i] = outer_product(normals[j],bubble_third_derivatives[aniso_indices[j]]);
        }
     if (fourth_derivatives.size() != 0)
        {
          fourth_derivatives[i] = outer_product(normals[j],bubble_fourth_derivatives[aniso_indices[j]]);
        }
  }
