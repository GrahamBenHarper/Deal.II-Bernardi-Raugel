...
FE_BernardiRaugel<dim>::FE_BernardiRaugel (const unsigned int p)
  ...
  FE_PolyTensor<PolynomialsBernardiRaugel<dim>, dim> (
    p,
    FiniteElementData<dim>(get_dpo_vector(p),
                           dim, 2, FiniteElementData<dim>::Hdiv),
    std::vector<bool>(PolynomialsBernardiRaugel<dim>::compute_n_pols(p), true),
    std::vector<ComponentMask>(PolynomialsBernardiRaugel<dim>::compute_n_pols(p),
...
FE_BernardiRaugel<dim>::get_dpo_vector (const unsigned int deg)
{
...
template <int dim>
void
FE_BernardiRaugel<dim>::initialize_support_points (const unsigned int deg)
{
  ...
  for(unsigned int i=0; i<dim; ++i)
  {
    for(unsigned int j=0; j<2; ++j)
    {
      ...
      unsigned int k = dim*GeometryInfo<dim>::vertices_per_cell + i*2 + j;
